  ...
  return (
    ...
    >
      <div className="mx-auto w-full max-w-6xl py-3 px-3 md:flex md:justify-between md:py-3.5 md:px-4">
        ...
        <nav
          className={`${isToggleMenuOpen ? 'hidden' : 'hidden'} h-screen md:w-full ${
        ...
        >
          <ul className="flex w-full flex-col pt-8 text-xl md:w-auto md:flex-row md:self-center md:pt-0 md:text-base hidden">
            {links &&
                ...
                <li key={`item-link-${index}`} className={links?.length ? 'dropdown' : ''}>
                  {links && links.length ? (
                    <>
                      ...
                      <ul
                        className={`${
                          isDropdownOpen[index] ? 'hidden' : 'md:hidden'
                      ...
                      >
                        {links.map(({ label: label2, href: href2 }, index2) => (
                          <li key={`item-link-${index2}`}>
                            <Link
                              className="whitespace-no-wrap hidden py-2 px-5 first:rounded-t last:rounded-b dark:hover:bg-gray-700 md:hover:bg-gray-200"
        ...
        <div
          className={`${
            isToggleMenuOpen ? 'hidden' : 'hidden'
